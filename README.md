...
- -----------------------------------------------
<A조=6명>김범주(3/2), 김수연(3/2), 김진아, 박정수, 배경득, 유우식,
<B조=5명>신승만, 이병현, 이용오(3/2), 한미래, 황초희,
<온라인=10명>
...
- redirect(새로고침해도 글이 재등록이 않됨) VS forward(새로고침하면 글이 중복등록이 됨) = 폭탄게시판이 만들어 집니다.
- index.jsp 파일을 수정해서 home폴더가 주 디자인으로 되게 변경.
